...
const Statistics = () => {
  ...
  const [stats, setStats] = useState({ avgTemp: 0, totalKicks: 0, avgHeartbeat: 0 });
  ...
  const loadStatistics = async () => {
    try {
      ...
      const groupedByDate = readings.reduce((acc: any, reading: any) => {
        ...
        if (!acc[date]) {
          acc[date] = { date, temps: [], kicks: [], heartbeats: [] };
        }
        if (reading.temperature) acc[date].temps.push(reading.temperature);
      ...
      const chartData = Object.values(groupedByDate).map((day: any) => ({
        date: day.date,
        temperature: day.temps.length > 0 
          ? (day.temps.reduce((a: number, b: number) => a + b, 0) / day.temps.length).toFixed(1)
          : 0,
      ...
      const allTemps = readings.filter(r => r.temperature).map(r => r.temperature);
    ...
    <Layout>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <Card className="p-5 rounded-3xl border-2 bg-gradient-to-br from-card to-primary-light/10 shadow-card">
            <Thermometer className="w-6 h-6 text-primary mb-2" />
            <p className="text-sm text-muted-foreground mb-1">Avg Temperature</p>
            <p className="text-2xl font-bold text-foreground">{stats.avgTemp.toFixed(1)}°C</p>
          </Card>
          ...
          <Card className="p-5 rounded-3xl border-2 bg-gradient-to-br from-card to-secondary/10 shadow-card col-span-2">
        ...
        <Card className="p-6 rounded-3xl border-2 bg-card shadow-card">
          ...
          <Tabs defaultValue="temperature" className="space-y-4">
            <TabsList className="grid w-full grid-cols-3 rounded-2xl">
              <TabsTrigger value="temperature" className="rounded-xl">Temperature</TabsTrigger>
